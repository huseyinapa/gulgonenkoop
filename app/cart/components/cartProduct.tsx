...
// Component prop tip tanımlaması
...
const CartProduct: React.FC<CartProductProps> = ({
  ...
  // Seçili ürünler ve sepetin tamamı için state'ler
  const [selected, setSelect] = useState<Product[]>(selectedItems);
  const [cartItems, setCartItems] = useState<Product[]>(cartProducts);
  ...
  useEffect(() => {
    ...
    // Eğer sepet verilerini sunucudan çekmek istersen burada çağırabilirsin.
  }, []);

  // Miktarı azaltan fonksiyon
  const handleDecreaseAmount = (itemId: string): void => {
    setSelect((prev) =>
      prev.map((item) =>
        item.pid === itemId
  ...
  // Miktarı artıran fonksiyon (ürünün stok bilgisine göre)
  const handleIncreaseAmount = async (itemId: string): Promise<void> => {
    ...
    setSelect((prev) =>
      prev.map((item) =>
        item.pid === itemId
          ? {
            ...item,
            amount: item.amount < product.stock! ? item.amount + 1 : item.amount,
          }
          : item
      )
    );
  ...
  const handleRemoveItem = async (pid: string): Promise<void> => {
    ...
    try {
      // Ürünü sepetten kaldır ve ilgili state'leri güncelle
      const updatedCartItems = cartItems.filter((x) => x.pid !== pid);
      const updatedSelectedItems = selectedItems.filter((x) => x.pid !== pid);
      const updatedSelect = selected.filter((x) => x.pid !== pid);

      setCartItems(updatedCartItems);
      setSelectedItems(updatedSelectedItems);
      setSelect(updatedSelect);

      // Eğer son ürünse, sepet uzunluğu için özel bir değer gönderiliyor
      if (cartItems.length <= 1) setLength(9991);

      // Sunucuya kaldırma isteği gönderiliyor
  ...
  // "Tümünü seç" checkbox'ı için event handler
            ...
            <div className="card-body justify-between items-center p-0">
              <div className="flex flex-col justify-between">
                <div className="flex flex-row justify-between">
                  <span className="font-medium lg:font-semibold">Ürünler:</span>
                  <span>{totalPrice} TL</span>
                </div>
                <div className="flex flex-row justify-between">
                  <span className="font-medium lg:font-semibold">Kargo ücreti:</span>
  ...
  // İç içe tanımlanan ProductCard component’i
        ...
        <figure className="relative">
          <img
            ...
            onError={(e) => {
              ...
              target.src = "/images/icons/shopping-bag.svg";
            }}
          />
